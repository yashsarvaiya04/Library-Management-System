...
namespace API.Controllers
{
    ...
    public class LibraryController : ControllerBase
    {
        ...
        {
            if (Context.Users.Any(u => u.Email.Equals(model.email) && u.Password.Equals(model.password)))
            {
                var user = Context.Users.Single(user => user.Email.Equals(model.email) && user.Password.Equals(model.password));
                ...
                return Ok( new { token=JwtService.GenerateToken(user),status=StatusCodes.Status200OK,user });
            }
